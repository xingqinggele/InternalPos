package com.example.internalpos.socket;
...
import static com.example.internalpos.net.Urls.socketUrls;
...
public class JWebSocketClientService extends Service {
    ...
    private Notification getNotification() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            //修改安卓8.1以上系统报错
            NotificationChannel notificationChannel = new NotificationChannel("mpn_msg", "mpn_msg_name", NotificationManager.IMPORTANCE_LOW);
            notificationChannel.enableLights(false);//如果使用中的设备支持通知灯，则说明此通知通道是否应显示灯
            notificationChannel.setShowBadge(false);//是否显示角标
            notificationChannel.setLockscreenVisibility(Notification.VISIBILITY_SECRET);
            NotificationManager manager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
            manager.createNotificationChannel(notificationChannel);
            builder.setChannelId("mpn_msg");
            builder.setContentTitle("\"众鑫助手内部版\"正在运行");
        }
